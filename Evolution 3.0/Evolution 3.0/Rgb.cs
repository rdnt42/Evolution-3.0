...
namespace Evolution_3._0
{
    class Rgb
    {
        static Random rnd = new Random(1974);
        static public int setX = 0;
        static public int setY = 0;
        /// <summary>
        ...
        public static byte[,,] GenerateBytePicture(int width, int height)
        {
        ...
        public static byte[,,] Test(int width, int height)
        {
            ...
            for (int c = 0; c < 3; ++c)
                for (int h = 0; h < height; ++h)
                    for (int w = 0; w < width; ++w)
                    {
                        if (w > setX && w < setX +300 && h > setY && h < setY+222)
                            result[c, h, w] = 255;
                        else
                            result[c, h, w] = 0;
        ...
        public unsafe static Bitmap RgbToBitmapTest(byte[,,] rgb)
        {
            if ((rgb.GetLength(0) != 3))
            {
                throw new ArrayTypeMismatchException("Size of first dimension for passed array must be 3 (RGB components)");
